...
graph TB
    ...
    Browser -->|HTTPS| Vercel["VERCEL EDGE NETWORK — meetingbingo.vercel.app"]
```
...
```mermaid
flowchart LR
    subgraph C["1. Card Generation"]
        c1["Select Category"] --> c2["Shuffle Words"] --> c3["Pick 24 Words"] --> c4["Render Card"]
    end

    subgraph S["2. Speech Recognition"]
        s1["Audio Input"] --> s2["Speech API"] --> s3["Transcript Text"] --> s4["Word Detection"]
        s4 --> s5["Match Found?"] --> s6["Fill Square"] --> s7["Update UI"]
    end

    subgraph W["3. Win Detection"]
        w1["Square Filled"] --> w2["Check Lines"] --> w3["BINGO? Yes!"] --> w4["Celebrate & Share"]
    end
```
...
```mermaid
graph TB
    Multi["FUTURE: MULTIPLAYER"]
    Multi --> A["Option A: WebSocket Server"]
    Multi --> B["Option B: Peer-to-Peer WebRTC"]
    Multi --> C["Option C: Firebase Realtime DB"]
    A --> A1["Real-time sync · Shared state · Leaderboard · $5–20/month"]
    B --> B1["No backend · Direct connections · ~10 players · Free"]
    C --> C1["Managed sync · Easy setup · 100 concurrent · Free"]
```
